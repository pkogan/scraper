...
// Descargar Totales por provincia
$totales=curl_local($urloficial.'00000000000000000000000b/1/1');
$json_totales=json_encode($totales,JSON_PRETTY_PRINT);
file_put_contents('salida/totales_'.date('ymd His ').'.json', $json_totales);
$provincias=[];

$provincia_row=[];
$provincia_row['scopeId']=$totales['id']['idAmbito']['scopeId'];
$provincia_row['name']=$totales['id']['idAmbito']['name'];
$provincia_row['codigo']=$totales['id']['idAmbito']['codigo'];
$provincia_row['envio']=$totales['id']['envio'];



foreach ($totales as $atributo=> $item) {
        if(!is_array($item)){
            $provincia_row[$atributo]=$item;
        }
};
foreach ($totales['partidos'] as $partido) {
       foreach ($partido['listas'] as $key => $lista) {
            $provincia_row[$partido['code'].$lista['nombre']]= $lista['votos'];    
        }
}
$provincias[]=$provincia_row;

foreach ($totales['mapa'][0]['scopes'] as $key => $provincia) {
    $provincia_row=[];
    foreach ($provincia as $atributo=> $item) {
        if(!is_array($item)){
            $provincia_row[$atributo]=$item;
        }
    };
...
$json_totales=json_encode($provincias,JSON_PRETTY_PRINT);
file_put_contents('salida/totales_provincias_'.date('ymd His ').'.json', $json_totales);
