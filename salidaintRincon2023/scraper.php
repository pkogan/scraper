<?php
$urloficial='https://rincondelossauces.datosoficiales.com/resultados/';
...
foreach ($ubicaciones as $key => $value) {
    if($value['clase_ubicacion']=='Mesa'){
        ...
        $url=$urloficial. str_replace('.', '/', $value['id_ubicacion']).'/CON.json';//'/INT.json';//'/CON.json';//'/INT.json';//'/CNJ.json';//'/DIP.json';//'/RPO.json';
