<?php
//$urloficial='https://neuquen.datosoficiales.com/resultados/';
$urloficial='https://resultados.tsje.gov.py/publicacion/dinamics/divulgacion.ajax.php?codeleccion=37&candidatura=1';
$str = file_get_contents($urloficial);
$resultados = json_decode($str, true); 
$jsonmesas=json_encode($resultados,JSON_PRETTY_PRINT);
$cont_mesas='XX';
$carpeta=dirname(__FILE__); 
//echo $carpeta; exit;
file_put_contents($carpeta.'/salida/'.date('ymd His ').'mesas '.$cont_mesas.'.json', $jsonmesas);
 
$hora=$resultados['horaFormated'];
$candidatos=$resultados['candidatos'];
$linea=$resultados['totales'];
